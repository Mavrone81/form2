...
- [ ] **Step 1:** `entry.js` imports `renderRecordPdf` with an esbuild alias mapping `pdfkit` to `pdfkit/js/pdfkit.es.js` (the browser ES build this pdfkit version ships; there is no pdfkit.standalone.js here) and a small virtual module replacing the `asset()` reads with embedded base64 (DejaVu ×3 + sRGB.icc, read from `assets/` at build time).
